...
import com.ardublock.ui.GeneratedCodeDialog;
...
public class GenerateCodeButtonListener implements ActionListener
{
	...
	private boolean forArduino;
	
	public GenerateCodeButtonListener(JFrame frame, Context context, boolean toArduino)
	{
		this.parentFrame = frame;
		this.context = context;
		workspace = context.getWorkspaceController().getWorkspace();
		uiMessageBundle = ResourceBundle.getBundle("com/ardublock/block/ardublock");
		this.forArduino=toArduino;
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		Set<RenderableBlock> initBlockSet = new HashSet<RenderableBlock>();
		...
		for (RenderableBlock renderableBlock:renderableBlocks)
		{
			...
			if (!block.hasPlug() && (Block.NULL.equals(block.getBeforeBlockID())))
			{
				...
				{
					initBlockSet.add(renderableBlock);
				}
		...
		try
		{
	/*		for (RenderableBlock renderableBlock : initBlockSet)
			{
				Block loopBlock = renderableBlock.getBlock();
				code.append(translator.translate(loopBlock.getBlockID(),true));
			}
	*/		
		...
		if (success)
		{
			...
			{
				GeneratedCodeDialog codeFrame=new GeneratedCodeDialog();
				codeFrame.textPane.setText(code.toString());
//				codeFrame.show();
				context.didPrintOnScreen(code.toString());
			}
